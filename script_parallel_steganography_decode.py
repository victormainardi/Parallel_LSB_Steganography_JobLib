...
if __name__ == "__main__":
 ...
 #   while(n_jobs_ <= 12):
   #     while(n <= 3):
            ...
            print(f"[DECODIFICAÇÃO PARALELA] [Tempo de Execução: {round(tempo_exec)} s ]","[Exec Nr: ", n, "] [n_jobs: ", n_jobs_,"]")
            media += tempo_exec
            n += 1
        media = int(media/3)
        print("[MEDIA DOS TEMPOS] [Tempo Médio: ", media, "s] com [n_jobs: ", n_jobs_, "]\n\n")
        n_jobs_ += 1
        n = 1
        media = 0
